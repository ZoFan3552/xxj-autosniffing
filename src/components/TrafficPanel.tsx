...
export function TrafficPanel({
  ...
  return (
    <div className="traffic-panel">
      ...
      {selected ? (
        <ResizableSplit direction="vertical" initialRatio={0.5} minRatio={0.2} maxRatio={0.85}>
          <TrafficTable records={records} selected={selected} onSelect={onSelect} parentRef={parentRef} rowVirtualizer={rowVirtualizer} />
...
function FlowStatus({ record }: { record: RequestRecord }) {
  if (record.status === "pending") return <span className="status-code status-pending">⏳</span>;
  if (record.status === "error") return <span className="status-code s5xx">ERR</span>;
  if (record.status === "aborted") return <span className="status-code status-aborted">ABT</span>;
  return <StatusCode code={record.response_status} />;
}

function StatusCode({ code }: { code: number | null }) {
  if (code == null) return <span className="status-code" style={{ color: "var(--text-2)" }}>—</span>;
  let cls = "status-code";
